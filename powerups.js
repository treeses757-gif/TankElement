...
function createPowerup(roomId, x, y, typeKey) {
  const powerupId = Date.now() + '_' + Math.random();
  const powerupRef = db.ref(`rooms/${roomId}/powerups/${powerupId}`);
  powerupRef.set({
    x: x,
    y: y,
    type: typeKey,
  ...
  setTimeout(() => {
    powerupRef.remove(); // auto-remove after 15 seconds if not picked
  }, 15000);
}

// Apply powerup effect to tank
function applyPowerup(tank, powerupType) {
  const p = powerupTypes[powerupType];
  if (p) p.effect(tank);
}